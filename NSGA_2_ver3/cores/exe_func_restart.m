...
ig_start = length(h_f_vec);
pop_vec = h_pop_vec{end};
f_vec = h_f_vec{end};
POP_NUM = size(pop_vec, 1)/2;


new_GENERATION = max(GENERATION, new_GENERATION);
new_GENERATION = new_GENERATION + 100;

for ig=ig_start+1:new_GENERATION    
...
%% 解のsave
save .\save\pop_data  h_pop_vec h_elite_pop_vec pop_rank h_f_vec new_GENERATION
